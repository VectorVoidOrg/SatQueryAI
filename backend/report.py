...
def _confidence_note(c):
    ...
    if c >= 0.8:
        ...
        band = "low"
    return (f"{c:.3f} ({band}) -- heuristic confidence estimate. Gemini does not "
            f"expose per-token logits, so this is a task-adjusted heuristic based "
            f"on answer length and task difficulty, not a calibrated correctness "
            f"probability.")
...
<div class="foot">
 ...
 Gemini VLM via the Google GenAI API. Confidence is a heuristic estimate
 (Gemini does not expose per-token logits). Visual evidence, where present,
 is a deterministic radiometric difference computed in pixel coordinates
 &mdash; it is <strong>not</strong> a model prediction and <strong>not</strong>
 georeferenced. Learned grounding (model-produced boxes or masks) is not
 implemented.</p>
...
def build_record(payload, report_id=None):
    ...
    rec["base_model"] = "gemini-3.6-flash"
    rec["limitations"] = [
        "confidence is a heuristic estimate, not a calibrated correctness probability",
