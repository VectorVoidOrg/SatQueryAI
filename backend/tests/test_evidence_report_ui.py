...
Everything here runs the REAL modules. Only the Gemini API call is stubbed
...
POLARITY = [
    ...
    ("north-west quadrant shows new buildings", None),
    ("Northwest expansion is visible.", None),
    ("yesterday's image shows more rooftops", None),
    ("notable expansion of the built-up area", None),
...
payload = {
    ...
    "confidence_source": "heuristic_estimate", "duration_seconds": 4.31,
    "inputs": [{"filename": "t1.png", "modality": "optical", "timestamp": "2023-01-01"}],
    "visual_evidence": EV,
    "auditable_execution_trace": [{"tool": "specialist_registry", "active_adapter": "gemini_vlm"}],
}
...
check("R3 build_record names the product and base model",
      rec["product"] == "SatQuery AI" and "gemini" in rec["base_model"].lower())
...
check("R11 confidence caveat printed next to the number",
      "NOT a calibrated probability" in doc or "heuristic" in doc.lower())
check("R12 evidence declared not a model prediction in the report body",
      "False" in doc and "is_model_prediction" not in doc)
...
check("R18 trace table rendered", "specialist_registry" in doc and "gemini_vlm" in doc)
