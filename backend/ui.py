...
<h1>SatQuery AI</h1>
<p class="sub">Agentic remote-sensing VLM &middot; Gemini 3.6 Flash via Google GenAI API</p>
...
$('go').addEventListener('click', async () => {
  ...
  try {
    ...
    out.innerHTML = `<div class="card"><div class="err">Request failed: ${esc(err.message)}
      <br><span class="muted">Is the backend running, and is the GEMINI_API_KEY set?</span></div></div>`;
  } finally {
    $('go').disabled = false;
  }
});
...
function render(b) {
  ...
  let html = `<div class="card">
    ...
    <p class="muted">Heuristic estimate &mdash; Gemini does not expose per-token logits, so
      this is a task-adjusted heuristic, <strong>not</strong> a calibrated probability of being correct.</p>
  </div>`;
